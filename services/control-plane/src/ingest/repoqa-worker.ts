 ...
 * this file (trace-bridge annotation, indexing/diagram payload types,
 * splitIdentifier/fuzzy matching, deterministic intent parser — ~280 lines)
 * now live in ./worker-helpers. The re-export bridge below keeps every
 * existing consumer (domain-radar-engine, repoqa-eval, this file's tests)
 * on the original import path; new code imports ./worker-helpers directly.
 */
import {
...
export class RepoQAWorker {
  ...
  /**
   * V27-30 (B3 increment 3) — diagram rendering (trace→mermaid, ADR-0013
   * layer instructions, session edge harvesting) moved verbatim to
   * ./worker-diagram. The two seams into worker state (symbol-graph index
   * lookup, start-symbol resolution shared with the query pipeline) are
   * injected; call sites below delegate through this.diagram unchanged in
   * behavior.
   */
  private readonly diagram = new WorkerDiagram({
    symbolIndexFor: (repoId) => this.getSymbolGraph(repoId).index,
    findStartSymbol: (question, symbols, explicitStart) =>
      this.findStartSymbol(question, symbols, explicitStart)
  });
  ...
  async *evolveRepo(input: {
    ...
    try {
      ...
      if (start) {
        ...
        if (trace.length >= 2) mermaid = this.diagram.traceToMermaid(trace, start.name);
      }
    } catch {
  ...
  async *queryRepo(input: {
    ...
    if (
      ...
      const real = await this.runReActLoop(
        ...
        (toolName, result) => this.diagram.harvestDiagramSession(diagramSession, toolName, result)
      );
      ...
      const engineMermaid = real.diagram
        ? this.diagram.renderLayerInstruction(real.diagram, repo, symbols, diagramSession)
        : undefined;
      if (engineMermaid) {
    ...
    if (usesSymbolResolution) {
      ...
      if (start) {
        ...
        mermaid = this.diagram.traceToMermaid(trace, start.name);
        if (startFallback) {
          this.repoqa.recordEvent({
            repoId: repo.id,
            eventType: 'tool.miss',
    ...
    if (isLlmConfigured(process.env)) {
      ...
      const real = await runReActAgent({
        ...
        onToolResult: (toolName, result) =>
          this.diagram.harvestDiagramSession(diagramSession, toolName, result)
      });
