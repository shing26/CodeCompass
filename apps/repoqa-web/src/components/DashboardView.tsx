      ...
      <div data-testid="dashboard-error" className="flex flex-1 items-center justify-center p-8">
        <div className="max-w-sm text-center">
          ...
          <button
            ...
            onClick={onRetry}
            className="mt-3 rounded-md border border-red-300 bg-white px-3 py-1 text-sm text-red-700 hover:bg-red-50"
          >
            重试
          </button>
        </div>
  ...
  if (!dashboard) {
    return (
      <div data-testid="dashboard-empty" className="flex flex-1 items-center justify-center p-8">
        <p className="text-sm text-slate-400">暂无看板数据。</p>
      </div>
    );
  }
      ...
      <div className="mx-auto max-w-4xl space-y-5">
        <header className="flex items-start justify-between gap-3">
          ...
          <button
            ...
            onClick={onOpenChat}
            className="shrink-0 rounded-md bg-accent px-3 py-1.5 text-sm font-medium text-white hover:bg-blue-700"
          >
            提问
          </button>
        </header>
          ...
          <div data-testid="scale" className="grid grid-cols-2 gap-2 sm:grid-cols-5">
            {SCALE_ORDER.map(({ key, label }) => (
              <div
                ...
                className="rounded-md border border-slate-100 bg-slate-50 px-2 py-1.5 text-center"
              >
                <div className="text-lg font-semibold text-slate-900">{dashboard.scale[key]}</div>
              ...
              {dashboard.config.topology.map((item: ConfigTopologyItem, idx: number) => (
                <li key={`${item.key}-${idx}`}>
                  <button
                    ...
                    onClick={() => onNavigate(item.filePath, item.lineStart ?? 1)}
                    className="flex w-full items-center gap-2 rounded-md px-1.5 py-1 text-left text-xs text-slate-700 hover:bg-slate-50"
                    title={item.filePath}
                  >
                    <span className="rounded bg-slate-100 px-1.5 font-mono text-[10px] text-slate-500">
                      {item.group}
                    </span>
                    <span className="truncate font-mono">{item.key}</span>
                    {item.sensitive && (
                      <span className="ml-auto shrink-0 rounded bg-red-50 px-1.5 text-[10px] text-red-600">
                        sensitive
                      </span>
                    )}
                  </button>
