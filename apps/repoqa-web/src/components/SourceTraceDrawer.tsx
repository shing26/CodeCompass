...
export function SourceTraceDrawer({ anchors, onNavigate }: SourceTraceDrawerProps) {
  ...
  return (
    <section data-testid="source-trace-drawer" className="mt-3 border-t border-slate-200 pt-3">
        ...
        {anchors.map((anchor, idx) => (
          <li key={`${anchor.file}:${anchor.line}:${idx}`}>
            <button
              ...
              className="w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-left hover:border-accent/40 hover:shadow-sm"
            >
              <div className="flex items-baseline justify-between gap-2">
                <span className="truncate font-mono text-xs text-slate-800">{anchor.file}</span>
                <span className="shrink-0 font-mono text-xs text-slate-400">L{anchor.line}</span>
              </div>
              {anchor.symbol && (
