...
| V27-30 | control-plane 结构轴手术（B 批主线，**进行中 2/4**）：✅增量 1 worker 头部工具外迁 worker-helpers.ts（95a2860，2556→2270）；✅增量 2 registerAnalysisRoutes 515 行巨注册拆 graph/gate/stream 三域+组成根（720e85f，e2e 62/62 逐端点护航）；⬜增量 3 diagram 簇抽 worker-diagram.ts 协作者（耦合已侦察：8 方法互调+读 this 符号图，接口先设计，见 v028 issues/03）；⬜增量 4 repoqa-* 目录归组（独立 commit，最后做）。零行为变更铁律，Mimosa SSRF 误报留痕在案 | 2026-09-14 grill D6/D8 + scan oversizedFiles 桶 | 结构 |
