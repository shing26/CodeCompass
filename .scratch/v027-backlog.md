...
| V27-30 | control-plane 结构轴手术（B 批主线，**1-3 段全闭、增量 4 拆票**）：✅增量 1 worker-helpers.ts 外迁（95a2860，2556→2270）；✅增量 2 analysis 515 行巨注册拆 graph/gate/stream（720e85f）；✅增量 3 diagram 簇 232 行抽 worker-diagram 协作者（DiagramContext 双缝注入，2556→2087，新增 4 契约钉，638/e2e 62 全绿，b23ba54）；⬜增量 4 repoqa-* 目录归组 churn 实测 ≈70 文件 → 拆 `.scratch/v028-engine-refactor/issues/04` 专注窗口执行（分组定稿+顺序+降级预案在案）。零行为变更铁律全程守住 | 2026-09-14 grill D6/D8 + scan oversizedFiles 桶 | 结构 |
