...
| [17](issues/17-ts-tour-anchors.md) | TS/JS 仓的 Tour 锚点族（**V27-17 升级票**；**✅ 已落地 2026-09-24**） | 11（同域 TS 解析） | 本仓 `get_tours` **由 `[]` 变两条真实路线**（中间件链/挂载链）；Java 侧逐字节不变；`App` 离榜（self 孤儿 246→243）；三仓复测留档 |
...
| [19](issues/19-multiline-template-masking.md) | 多行模板串未掩码 → 幻影声明（**✅ 已落地 2026-09-24**；票面拟的"两阶段"实测改为**单趟状态机**——配对只有在注释被排除后才可信，论证在票 19 §3/§4） | 18（同文件，已串行） | 多行模板里的 `interface X {` 不得产出符号（4 用例先红后绿）；同树对照幻影 15 个归零（2045→2030，孤儿未上升）；棘轮复算 12.1% 未动 baseline |
...
**19/20/21 = Wave 6**（2026-09-24 由票 18 实测派生，**当日全部落地**：19 = 多行模板未掩码 → 单趟状态机掩码器；20 = 接口→实现表（前提被实测推翻，实为接收者定型三处缺陷）；21 = 按名回退假边口子（先量三仓 1014 条假边再修，修后 0/0/0）。三票与票 18 同文件面，已全部串行交付）。
