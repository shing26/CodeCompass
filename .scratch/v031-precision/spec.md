...
| [20](issues/20-interface-implementation-table.md) | 接口→实现关系表（**ADR-0018 `deferred` 的 A′ step 2**；票 18 复测后流订阅族露面，2026-09-24 立项） | 18（同文件，须串行） | 唯一实现即绑定、多实现 fail-closed；self 流订阅族离榜；lazygit 620 条 `deferred` 计数下降 |
...
**19/20 = Wave 6**（2026-09-24 由票 18 实测派生：19 = 多行模板未掩码的幻影声明；20 = 接口→实现关系表，ADR-0018 `deferred` 的 A′ step 2。两票均与票 18 同文件面，须串行）。
