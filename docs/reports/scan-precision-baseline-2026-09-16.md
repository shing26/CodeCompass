...
## 14. 第九增量（票 19：掩码器改单趟状态机，2026-09-24）

`maskLiteralsAndComments` 的 `'`/`"` 分支按行（字符类排除换行），**多行模板串只被掩到第一个换行**，其后内容被当源码扫描
→ `interface X {` 之类的文字变成**幻影符号**。两天内咬了两次（票 18 fixture 的 `interface RepoContextValue`、票 20 fixture 的
`class RepoQAClient`），都与真声明同名异内容，被跨文件表的歧义规则（正确地）丢弃真表项，症状一律是"**改了没用**"。
生产代码同样暴露（`chat/agent.ts` 提示词、`engine/repoqa-export.ts` 导出模板都是多行模板串）。

**一行加宽 backtick 分支会自伤（实测）**：掩码器自身的源码与注释里就有反引号，跨行匹配后注释里的反引号会与远处的反引号配对，
**中间的真注释文字反而不被掩码**——本仓 `TypeScriptAdapter.ts` 当场把它注释里那句 `interface RepoContextValue { … }`
变成幻影声明（探针 `probe-mask.ts` 输出 `masked@29859 line=760`）。**配对只有在注释被排除之后才可信**，
是状态机问题不是正则强度问题。

**落地**：单趟扫描式状态机（同长空格替换，偏移契约不变），六类字面量各走各的状态：注释、`'`/`"`、模板串（允许跨行 +
`${…}` 插值 + 插值内嵌套模板）、正则字面量（前导字符判"除法 vs 正则"、遇换行判非正则、字符类内 `/` 不闭合）。
**已知边界刻意保守**：`)`/`]` 之后的 `/` 一律当除法（`if (x) /re/.test(y)` 不被掩码）——少掩码只可能造幻影，多掩码会藏掉真代码。
...
→ **幻影 15 个**（且这是在**新增 4 条单测**的前提下净减）；孤儿**未上升**；lazygit 10523/1828、petclinic 595/13 逐字节不变
（Go/Java 不走这个掩码器）。棘轮复算 self **246/2030 = 12.1%**（分母变小使比值自 12.0% 被动上升），天花板 17.1% 仍在界内，
**未动** `--ratchet-update`。门禁：控制面 722、web 364、bridge 26、e2e 71/0、97 题 eval 九桶全 100%。
