...
5. vitest 并发抖动：瞬时失败复跑两次确认再定性（历史规律：复跑即绿）。**同族三例**（前两例 2026-09-21，第三例 2026-09-24）：① 前端 `npm run build` 在内存吃紧时 `vite:worker Failed to allocate memory`（monaco 的 ts.worker 6MB 块）→ `NODE_OPTIONS=--max-old-space-size=6144 npm run build` 即过；② gate 在机器换页时可能 `server did not become healthy` → 复跑即绿；③ **CI 的 windows-latest 上 `db.test.ts > rotates backups to the newest five` 会撞 5000ms 默认超时**（该文件自 v0.3.0 起未动；本地连跑 3/3 绿、`gh run rerun --failed` 后 5 job 全绿）——SQLite 备份轮转是真文件 I/O，Windows runner 偶发偏慢。三者都不是代码问题，但**发布前 `prepublishOnly` 会跑全量构建**，撞上时别误判为坏包
...
| 03 发布就绪 | ✅ **已发布（2026-09-25，M4=1）**——`@codecompass/cli@0.31.0` 上 npm；干净目录一行命令实测 MCP 握手（17 工具/list_repos 应答）；发布拦路（scope 未建 / 2FA 政策 / PowerShell `&&`）与解法已入档票 03；**v0.31 批收口判据（M1 诚实下限 + M4=1 + M5≥3）全部达成** |；**09-25 双轴评审已补**（`docs/reports/code-review-2026-09-25.md`，11 条处置：3 修 + M1 重判 90%→40% + spec 回写 + 6 登记）；tag/Release 重指至 `142fbb6`（评审批终态），npm@0.31.0 tarball 为评审前构建（差异三处小改随 v1.0.0 进 npm，留痕票 03）
